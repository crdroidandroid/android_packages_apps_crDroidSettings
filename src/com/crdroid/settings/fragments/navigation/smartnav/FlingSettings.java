...
import com.crdroid.settings.fragments.navigation.smartnav.ActionPreference;
import com.crdroid.settings.fragments.navigation.smartnav.IconPickHelper;
import com.crdroid.settings.fragments.navigation.smartnav.IconPickHelper.OnPickListener;
...
public class FlingSettings extends ActionFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mTrailsEnabled.setChecked(Settings.Secure.getIntForUser(resolver,
                Settings.Secure.FLING_TRAILS_ENABLED, 1, UserHandle.USER_CURRENT) == 1);
        mTrailsEnabled.setOnPreferenceChangeListener(this);

        int trailsColor = Settings.Secure.getIntForUser(resolver,
        ...
        int width = Settings.Secure.getIntForUser(resolver,
    ...
    private void resetFling() {
        ...
        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LOGO_VISIBLE, 1, UserHandle.USER_CURRENT);
        mShowLogo.setChecked(true);
        mShowLogo.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LOGO_ANIMATES, 1, UserHandle.USER_CURRENT);
        mAnimateLogo.setChecked(true);
        mAnimateLogo.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_RIPPLE_ENABLED, 1, UserHandle.USER_CURRENT);
        ...
        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_RIGHT_PORT, mIsTablet ? 30 : 40, UserHandle.USER_CURRENT);
        mSwipePortRight.setValue(mIsTablet ? 30 : 40);
        mSwipePortRight.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_LEFT_PORT, mIsTablet ? 30 : 40, UserHandle.USER_CURRENT);
        mSwipePortLeft.setValue(mIsTablet ? 30 : 40);
        mSwipePortLeft.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_RIGHT_LAND, 25, UserHandle.USER_CURRENT);
        mSwipeLandRight.setValue(25);
        mSwipeLandRight.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_LEFT_LAND, 25, UserHandle.USER_CURRENT);
        mSwipeLandLeft.setValue(25);
        mSwipeLandLeft.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_UP_LAND, 40, UserHandle.USER_CURRENT);
        mSwipeVertUp.setValue(40);
        mSwipeVertUp.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_LONGSWIPE_THRESHOLD_DOWN_LAND, 40, UserHandle.USER_CURRENT);
        mSwipeVertDown.setValue(40);
        mSwipeVertDown.setOnPreferenceChangeListener(this);

        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_KEYBOARD_CURSORS, 1, UserHandle.USER_CURRENT);
        ...
        Settings.Secure.putIntForUser(getContentResolver(),
                Settings.Secure.FLING_TRAILS_ENABLED, 1, UserHandle.USER_CURRENT);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (preference == mShowLogo) {
            ...
            return true;
        } else if (preference == mTrailsEnabled) {
            boolean enabled = ((Boolean) newValue).booleanValue();
            Settings.Secure.putIntForUser(resolver,
                    Settings.Secure.FLING_TRAILS_ENABLED, enabled ? 1 : 0, UserHandle.USER_CURRENT);
            return true;
        } else if (preference == mTrailsColor) {
            int color = ((Integer) newValue).intValue();
            Settings.Secure.putIntForUser(resolver,
                    Settings.Secure.FLING_TRAILS_COLOR, color, UserHandle.USER_CURRENT);
            return true;
    ...
    public static void reset(Context mContext) {
        ...
        Settings.Secure.putIntForUser(resolver, Settings.Secure.FLING_LONGSWIPE_THRESHOLD_RIGHT_PORT,
             ActionUtils.navigationBarCanMove() ? 40 : 30, UserHandle.USER_CURRENT);
        ...
        Settings.Secure.putIntForUser(resolver, Settings.Secure.FLING_TRAILS_ENABLED,
             1, UserHandle.USER_CURRENT);
        Settings.Secure.putIntForUser(resolver, Settings.Secure.FLING_TRAILS_COLOR,
             Color.WHITE, UserHandle.USER_CURRENT);
        Settings.Secure.putIntForUser(resolver, Settings.Secure.FLING_TRAILS_WIDTH,
